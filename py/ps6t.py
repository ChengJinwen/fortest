...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        """
        a_c = self.names_to_nodes[a]
        b_c = self.names_to_nodes[b]
        a_parent = []
        b_parent = []
        res = (-1,0)
        while a_c.get_parent() != None:
            a_parent.insert(0,a_c.parent.name)
            a_c = a_c.parent
        while b_c.get_parent() != None:
            b_parent.insert(0,b_c.parent.name)
            b_c = b_c.parent 
        if (a in b_parent) or (b in a_parent) or a_parent == [] or b_parent == []:
            lrem = abs(len(b_parent)-len(a_parent))
            return (-1,lrem)
        elif a == b:
            return res
        else:
            ...
            lmax = max(la,lb) 
            n = 0
            while n <= lmin-1 and a_parent[n] == b_parent[n]:
                n += 1
            return (lmin-n,lmax-lmin)
...
t, r = f.cousin("h", "a")
print " is a", words[t],"cousin", r, "removed from "
t, r = f.cousin("d", "f")
